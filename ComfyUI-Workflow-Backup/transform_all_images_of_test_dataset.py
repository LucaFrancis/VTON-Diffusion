...
def main():
    ...
    with torch.inference_mode():
        ...
        seargeinput7_150 = seargeinput7.mux(
            lora_strength=0.0, operation_mode="inpainting", prompt_style="simple"
        )
        ...
        df = pd.read_csv("/home/luca/virtual-tryon-diffusion/datasets/query_list.csv")
        ...
        for path in tqdm(df["image_name"]):
            ...
            loadimage_1 = loadimage.load_image(
                image=dataset_path + path)
            im = Image.open(dataset_path + path)
            width, height = im.size
            print(width, height)
            loadimage_mask = loadimage.load_image(
                image=dataset_path + (path.split(".")[0] + "_mask_sam.png"))
            ...
            seargeimagesave.save_images(
                filename_prefix="transform_all/" + run + "/testimage_mask.png",
                images=get_value_at_index(imagescale_284, 0),
                state=get_value_at_index(seargeoutput2_359, 8),
                save_to=get_value_at_index(seargeoutput2_359, 9),
            )
            seargeimagesave.save_images(
                filename_prefix="transform_all/" + run + "/testimage.png",
                images=get_value_at_index(imagescale_281, 0),
                state=get_value_at_index(seargeoutput2_359, 8),
                save_to=get_value_at_index(seargeoutput2_359, 9),
            )
            ...
            vaedecode_ganggang = vaedecode.decode(
                samples=get_value_at_index(vaeencode_282, 0),
                vae=get_value_at_index(seargevaeloader_351, 0),
            )
            seargeimagesave.save_images(
                filename_prefix="transform_all/" + run + "/testimage_vae.png",
                images=get_value_at_index(vaedecode_ganggang, 0),
                state=get_value_at_index(seargeoutput2_359, 8),
                save_to=get_value_at_index(seargeoutput2_359, 9),
            )
            ...
            seargesdxlsampler2_358 = seargesdxlsampler2.sample(
                ...
                latent_image=get_value_at_index(setlatentnoisemask_283, 0),
