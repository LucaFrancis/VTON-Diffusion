...
def main():
    ...
    with torch.inference_mode():
        ...
        vaeencodeforinpainting = NODE_CLASS_MAPPINGS["VAEEncodeForInpaint"]()
        ...
        for root, _, files in os.walk(
                "/home/luca/ComfyUI/models/embeddings"):
            for f in files:
                if f.lower().startswith(embedding_name + '-'):
                    embedding_checkpoints.append(f)
        print("found", len(embedding_checkpoints), "embedding training checkpoints")

        image_path = "/home/luca/virtual-tryon-diffusion/datasets/deep-fashion/In-shop Clothes Retrieval Benchmark/Img/img_highres/MEN/Jackets_Vests/id_00000094/01_1_front.jpg"

        loadimage_268 = loadimage.load_image(image=image_path)

        loadimage_mask = loadimage.load_image(
            image=image_path.split(".")[0] + "_mask_sam.png")
        ...
        source_image_path = '/home/luca/ComfyUI/output/face_study/' + run + "/source_image.txt"
        os.makedirs(os.path.dirname(source_image_path), exist_ok=True)
        with open(source_image_path, 'w') as f:
            f.write(image_path)

        checkpoint = "init_will_be_skipped"
        ...
        evaluation_images = [
            "/home/luca/virtual-tryon-diffusion/datasets/deep-fashion/In-shop Clothes Retrieval Benchmark/Img/img_highres/WOMEN/Tees_Tanks/id_00006777/01_1_front.jpg",
            "/home/luca/virtual-tryon-diffusion/datasets/deep-fashion/In-shop Clothes Retrieval Benchmark/Img/img_highres/WOMEN/Tees_Tanks/id_00007708/03_1_front.jpg",
            "/home/luca/virtual-tryon-diffusion/datasets/deep-fashion/In-shop Clothes Retrieval Benchmark/Img/img_highres/WOMEN/Blouses_Shirts/id_00007072/02_1_front.jpg",
            "/home/luca/virtual-tryon-diffusion/datasets/deep-fashion/In-shop Clothes Retrieval Benchmark/Img/img_highres/WOMEN/Sweatshirts_Hoodies/id_00007908/01_1_front.jpg",
            "/home/luca/virtual-tryon-diffusion/datasets/deep-fashion/In-shop Clothes Retrieval Benchmark/Img/img_highres/MEN/Jackets_Vests/id_00000084/08_1_front.jpg",
        ]

        for checkpoint in tqdm(embedding_checkpoints):
            for evaluation_image in tqdm(evaluation_images):
                loadimage_1 = loadimage.load_image(
                    image=evaluation_image)
                loadimage_mask = loadimage.load_image(
                    image=evaluation_image.split(".")[0] + "_mask_sam.png")
                mask = imagetomask.image_to_mask(
                    channel="green", image=get_value_at_index(loadimage_mask, 0)
                )

                seargeinput1_270 = seargeinput1.mux(
                    main_prompt="(embedding:" + checkpoint + ") Model photoshoot for clothing retailer website of nice looking "+embedding_name+"  with a clear background, beautiful, best quality, high definition, studio lighting, white background, gradient background (masterpiece), (extremely intricate), deviantart hd, artstation hd, detailed face, award-winning photography, margins, detailed face, intricate, high detail, sharp focus, dramatic, award winning matte cinematic lighting volumetrics dtx",
                    secondary_prompt=get_value_at_index(seargeprompttext_5, 0),
                    style_prompt=get_value_at_index(seargeprompttext_6, 0),
                    negative_prompt=get_value_at_index(seargeprompttext_7, 0),
                    negative_style=get_value_at_index(seargeprompttext_8, 0),
                ...
                print("Saving image for ", checkpoint)
                seargeimagesave.save_images(
                    filename_prefix="face_study/" + run + "/" + checkpoint,
